...
class avoidance(threading.Thread):
    ...
    def __init__(self, robot_ip, robot_port=9559):
        ...
        # assert self.video_client is not None  手动注释
    ...
    def say(self, bodies):
        ...
        for b in bodies[1:]:
            ...
            if (b.find('Head') == 0):
                ...
                elif b.find('Middle') != -1:
                    print "2"
                    self.motionProxy.moveInit()
                    global isBegin
                    isBegin = 2    # C出口
                    self.tts.say ("on the way")
                elif b.find('Rear') != -1:
                    print "3"
                    self.motionProxy.moveInit()
                    global isBegin
                    isBegin = 3   # C入口
                    self.tts.say("on your way")
                elif b.find('RHand') != -1:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ip", type=str, default="192.168.2.103", help="Robot ip address")
